...
def main(args):
    ...
    with open(os.path.join(args.data_path, 'conll/Paragraph_Level/all.dat')) as f:
        for is_line, lines in itertools.groupby(f, key=lambda x: x != "\n"):
            if is_line:
                ...
                paragraph_text = " ".join([token.split("\t")[1] for token in paragraph])

                for is_AC, tokens in itertools.groupby(paragraph,
                                    key=lambda x: not x.split("\t")[4].startswith("O")):
                    tokens = list(tokens)
                    indices = [int(token.split("\t")[0]) -
                               1 for token in tokens]
                    text = " ".join([token.split("\t")[1] for token in tokens])
                    tag = list(set([token.split("\t")[4].lstrip("BI-") for token in tokens]))
                    assert len(tag) == 1
                ...
                paragraph = Paragraph(paragraph_text, ACs, stance, paragraph_length)
    ...
    with open(os.path.join(args.data_path, 'conll/Essay_Level/all.dat')) as f:
        for is_line, lines in itertools.groupby(f, key=lambda x: x != "\n"):
            if is_line:
                lines = list(lines)
    ...
    for paragraph_i, paragraph in enumerate(paragraphs):
        ...
        if essays[essay_i].length <= essay_length + 5:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--tag", action="store_true", default=True)
    parser.add_argument("--data-path", type=str, \
                        default='data/acl2017-neural_end2end_am/data/')
